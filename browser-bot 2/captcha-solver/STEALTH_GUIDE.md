...
### 4. Блокировка только СТОРОННИХ трекеров

Глобальный route abort:
- `**/*googletagmanager*`
- `**/*google-analytics*`
- `**/*doubleclick*`
- `**/*facebook.net*`
...
**Что важно — Яндекс-домены НЕ блокируем.** В частности:
- `mc.yandex.ru` — Yandex.Metrika
- `an.yandex.ru`
- `yastatic.net/metrika*`

Эти домены передают телеметрию движений мыши, кликов и скроллов на сервер Яндекса. **SmartCaptcha анализирует эти данные** при принятии решения «человек или бот». Если их заблокировать — клик по галочке «Я не робот» не подтверждается на серверной стороне, капча зависает на чекбоксе и не эскалирует до картинок. Парсер ловит таймаут.

Если в вашем парсере метрика блокировалась самостоятельно (через uBlock-list или роуты) — отключите блокировку **именно для капча-страниц** (URL содержит `/showcaptcha` или `/captcha`).
